...
case "$MODULE" in
    ...
    bkiam_engine|iam_engine)
        ...
        ;;
    appo|appt)
        echo "sync $BK_PKG_SRC_PATH/paas_agent/ to module: $1" 
        "${SELF_DIR}"/sync.sh "$MODULE" "$BK_PKG_SRC_PATH/paas_agent/" "$BK_PKG_SRC_PATH/paas_agent/"
        echo "sync $BK_PKG_SRC_PATH/image/ to module: $1" 
    ...
    job)
        ...
        bkiam_migrate job
        # 更新后端
        echo "update job backend on all job hosts"
        ...
        # 更新前端
        echo "update job frontend on nginx hosts"
        ...
        _update_common_info "${MODULE}"
        ;;
    usermgr|bknodeman|bkmonitorv3|bklog|fta)
        echo "sync $MODULE to remote hosts"
        "$SELF_DIR"/sync.sh "${MODULE#bk}" "$BK_PKG_SRC_PATH/$MODULE_DIR_NAME/" "$BK_PKG_SRC_PATH/$MODULE_DIR_NAME/"
        ...
        if [[ "${MODULE}" =~ "bknodeman"  ]]; then
            # 更新后刷新nginx上的脚本
            $PCMD -H "${BK_NODEMAN_IP}" 'workon bknodeman-nodeman; runuser -u blueking ./bin/manage.sh copy_file_to_nginx'
        fi
        _update_common_info "${MODULE}"
        ;;
    cert)
    ...
    lesscode)
        ...
        emphasize "update lesscode  on lesscode hosts"
