...
_initdata_nodeman () {
    ...
    gse_pkg=$(_find_latest_gse)
    emphasize "pack gse client with plugin"  
    bash ${CTRL_DIR}/bin/pack_gse_client_with_plugin.sh \
        -f ${BK_PKG_SRC_PATH}/${gse_pkg} -p ${BK_PKG_SRC_PATH}/gse_plugins -e ${CTRL_DIR}/bin/04-final/gse.env -c "${INSTALL_PATH}"/cert 
...
_initdata_cmdb () {
    ...
    cat "${tmpfile}" | jq
    if ! [[ $(cat $tmpfile | jq .result) =~ 'true' ]]; then
