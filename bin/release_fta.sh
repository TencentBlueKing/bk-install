...
# 渲染配置用的环境变量文件
ENV_FILE=${SELF_DIR}/04-final/fta.env
...
# 备份老的包，并解压新的
tar -czf "$BACKUP_DIR/fta_$(date +%Y%m%d_%H%M).tgz" -C "$PREFIX" fta etc/supervisor-fta-fta.conf
...
if [[ $RELEASE_TYPE = tgz ]]; then
    ...
    # 更新全局文件
    log "updating 全局文件 ..."
    rsync -a --delete "${TMP_DIR}/fta/" "$PREFIX/fta/"
else
    rsync -a --delete "${MODULE_SRC_DIR}/fta/" "$PREFIX/fta/"
fi

# 安装虚拟环境和pip包
"${SELF_DIR}"/install_py_venv_pkgs.sh -p "$PYTHON_PATH" \
    -n "fta-fta" \
    -w "${PREFIX}/.envs" -a "$PREFIX/fta/fta" \
    -r "$PREFIX/fta/fta/requirements.txt"

# 渲染配置
if [[ $UPDATE_CONFIG -eq 1 ]]; then
    source /etc/blueking/env/local.env
    source "$BK_HOSTS_ENV"
    "$SELF_DIR"/render_tpl -u -m fta -p "$PREFIX" \
        -e "$ENV_FILE" \
        -E LAN_IP="$LAN_IP" \
        -E BK_BEANSTALK_IP_COMMA="$BK_BEANSTALK_IP_COMMA" \
        "$PREFIX"/fta/support-files/templates/*
else
    # 走固定配置从$PREFIX/etc下拷贝回去
    if [[ -d "$PREFIX"/etc/fta ]]; then
        rsync -av "$PREFIX"/etc/fta/ "$PREFIX"/fta/fta/
    fi
fi

# 重启进程
/opt/py36/bin/supervisorctl -c "$PREFIX"/etc/supervisor-fta-fta.conf reload