...
readarray -t MONITOR_ENABLED_MODULE < \
    <(systemctl list-unit-files --state=enabled --type=service \
        | awk '/^bk-(monitor|transfer|influxdb-proxy|grafana).service/ { sub(".service","",$1); print $1 }')
if (( ${#MONITOR_ENABLED_MODULE[@]} == 0 )); then 
    warning "there is no enabled bk-(monitor|transfer|influxdb-proxy|grafana) systemd service on this host"
fi
### 文件/文件夹是否存在判断
# 通用的变量
...
if [[ $RELEASE_TYPE = tgz ]]; then
    ...
    # 更新全局文件
    log "updating 全局文件 ..."
    rsync -a --delete "${TMP_DIR}/bkmonitorv3/" "$PREFIX/bkmonitorv3/"
else
