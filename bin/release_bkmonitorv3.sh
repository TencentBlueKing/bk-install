...
readarray -t MONITOR_ENABLED_MODULE < \
    <(systemctl list-unit-files --state=enabled --type=service \
        | awk '/^bk-(monitor|transfer|influxdb-proxy|grafana|unify-query|argus-api|argus-receive|argus-storegw|argus-comact).service/ { sub(".service","",$1); print $1 }')
if (( ${#MONITOR_ENABLED_MODULE[@]} == 0 )); then 
    warning "there is no enabled bk-(monitor|transfer|influxdb-proxy|grafana|unify-query|argus-api|argus-receive|argus-storegw|argus-comact) systemd service on this host"
fi
### 文件/文件夹是否存在判断
# 通用的变量
...
rsync -a "${SRC_DIR}/project.yaml" "${SRC_DIR}/VERSION" "$PREFIX/bkmonitorv3/"
...
for m in "${UPDATE_MODULE[@]}"; do
    if [[ $m = "bk-monitor" ]]; then
        # 安装虚拟环境和依赖包
        ${SELF_DIR}/install_py_venv_pkgs.sh -e -n "bkmonitorv3-monitor" \
            -p "/opt/py36/bin/python" \
            -w "${PREFIX}/.envs" -a "$PREFIX/bkmonitorv3/monitor" \
            -r "$PREFIX/bkmonitorv3/monitor/requirements.txt" \
            -s "${PREFIX}/bkmonitorv3/support-files/pkgs"
    fi
