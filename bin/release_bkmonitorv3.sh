...
readarray -t MONITOR_ENABLED_MODULE < \
    <(systemctl list-unit-files --state=enabled --type=service \
        | awk '/^bk-(monitor|transfer|influxdb-proxy|grafana|unify-query|argus-api|argus-receive|argus-storegw|argus-comact|ingester).service/ { sub(".service","",$1); print $1 }')
if (( ${#MONITOR_ENABLED_MODULE[@]} == 0 )); then 
    warning "there is no enabled bk-(monitor|transfer|influxdb-proxy|grafana|unify-query|argus-api|argus-receive|argus-storegw|argus-comact|ingester) systemd service on this host"
fi
### 文件/文件夹是否存在判断
# 通用的变量
