...
"${SELF_DIR}"/install_py_venv_pkgs.sh -e -p "$PYTHON_PATH" \
    -n "bklog-api" \
    -w "${PREFIX}/.envs" -a "$PREFIX/bklog/api" \
    -r "$PREFIX/bklog/api/requirements.txt" \
    -s "${PREFIX}/bklog/support-files/pkgs"
...
# 重启进程
/opt/py36/bin/supervisorctl -c "$PREFIX"/etc/supervisor-bklog-api.conf reload