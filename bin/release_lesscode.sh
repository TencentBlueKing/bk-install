...
# 渲染配置用的环境变量文件
ENV_FILE=${SELF_DIR}/04-final/lesscode.env 
...
# 备份现网目录
tar -C "$PREFIX" -czf "$BACKUP_DIR/lesscode-$(date +%Y%m%d_%H%M).tgz"  lesscode
...
if [[ $RELEASE_TYPE = tgz ]]; then
    ...
    # 更新全局文件
    log "updating 全局文件 ..."
    rsync --delete -av "${TMP_DIR}"/lesscode/ "$PREFIX"/lesscode/
else
    rsync --delete -av "${MODULE_SRC_DIR}/lesscode/" "$PREFIX"/lesscode/
fi

# 渲染配置文件
if [[ $UPDATE_CONFIG = 1 ]]; then
    $RENDER_TPL -u -p "$PREFIX" -m lesscode -e "$ENV_FILE" "$PREFIX/lesscode/support-files/templates"/*
fi

cd "${INSTALL_PATH}"/lesscode && npm run build

chown blueking.blueking -R "$PREFIX"/lesscode
systemctl restart bk-"${MODULE}".service

# 检查本次更新的模块启动是否正常
running_status=()
for m in "${UPDATE_MODULE[@]}"; do
    running_status+=( "$(systemctl show -p SubState,MainPID,Names bk-"$m".service | awk -F= '{print $2}'  | xargs)" )
done
