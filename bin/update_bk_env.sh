...
DEFAULT_BK_UID=10000
DEFAULT_BK_GID=10000

# create blueking user and group
getent group blueking &>/dev/null || \
    groupadd --gid "$DEFAULT_BK_GID" blueking
id -u blueking &>/dev/null || \
    useradd --uid "$DEFAULT_BK_UID" --gid "$DEFAULT_BK_GID" -m -d /home/blueking -c "BlueKing EE User" --shell /bin/bash blueking 
...
# 安装基础命令和基础包
yum -y install rsync jq expect uuid bash-completion lsof
yum -y install openssl-devel readline-devel libcurl-devel libxml2-devel glibc-devel zlib-devel iproute sysvinit-tools procps-ng bind-utils
