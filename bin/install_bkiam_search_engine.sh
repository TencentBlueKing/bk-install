#!/usr/bin/env bash
# 用途：安装和更新蓝鲸的bkiam_search_engine，权限中心后台引擎
# 
...
用法: 
    $PROGRAM [ -h --help -?  查看帮助 ]
            ...
            [ -s, --srcdir      [必填] "从该目录拷贝bkiam_search_engine/目录到--prefix指定的目录" ]
            [ -p, --prefix      [可选] "安装的目标路径，默认为/data/bkee" ]
            [ -l, --log-dir     [可选] "日志目录,默认为$PREFIX/logs/bkiam_search_engine" ]
            [ -d, --data-dir    [可选] "数据目录,默认为$PREFIX/public/bkiam_search_engine" ]
...
LOG_DIR=${LOG_DIR:-$PREFIX/logs/bkiam_search_engine}
DATA_DIR=${DATA_DIR:-$PREFIX/public/bkiam_search_engine}
...
# 拷贝模块目录到$PREFIX
rsync -a "$MODULE_SRC_DIR/bkiam_search_engine" "$PREFIX/" || error "安装模块(bkiam_search_engine)失败"
chown -R blueking.blueking "$PREFIX/bkiam_search_engine"
