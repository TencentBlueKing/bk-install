...
PIP_OPTIONS=(--no-cache-dir --no-use-pep517)
ENCRYPT=
[[ -n "$CTRL_DIR" ]] || CTRL_DIR=$(readlink -f $SELF_DIR/../)
...
set -e
if [[ -d "$PKG_PATH" ]];then
   PIP_OPTIONS+=("--find-links=$PKG_PATH" --no-index --no-cache-dir)
fi

"${WORKON_HOME}/$VENV_NAME/bin/pip" install -r "$REQ_FILE" "${PIP_OPTIONS[@]}"

version=$("$WORKON_HOME/$VENV_NAME"/bin/python --version 2>&1 | grep -Eo '([0-9])+(\.)+[0-9]')
if [[ "${version}" =~ 3.6 ]]; then
    printf -v python_string "python%s" "${version}"
    sed -i "1s,bin/python[0-9. ]*$,bin/$python_string," "${WORKON_HOME}/$VENV_NAME/bin/pip" 
fi