...
echo bklog bkmonitorv3 bknodeman cmdb job paas usermgr apigw | xargs -n1 ./bkcli render
# job frontend
...
echo paas usermgr cmdb job apigw | xargs -n1 "${SELF_DIR}"/../bkcli restart
sleep 15
echo paas cmdb job | xargs -n1 "${SELF_DIR}"/../bkcli check
echo bklog bkmonitorv3 bknodeman | xargs -n1 "${SELF_DIR}"/../bkcli restart
