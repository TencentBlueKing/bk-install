...
case $MODULE_DIR_NAME in 
    open_paas)   # 因为env的文件名是paas.env
        ...
        ;;
    usermgr|bkiam|bkssm|bkmonitorv3|bklog|bkauth)
        # 渲染配置
    ...
    bkiam_search_engine|iam_search_engine)
        "$SELF_DIR"/render_tpl -u -e "${SELF_DIR}/04-final/${MODULE_DIR_NAME}.env" \
           -E LAN_IP="$LAN_IP" -m "$MODULE_DIR_NAME" -p "$BK_HOME" \
           "$BK_PKG_SRC_PATH"/"$MODULE_DIR_NAME"/support-files/templates/*
        ;;
    bk_apigateway)
        "$SELF_DIR"/render_tpl -u -m "$MODULE_DIR_NAME" -p "$BK_HOME" \
          -E LAN_IP="$LAN_IP" -e "${SELF_DIR}/04-final/bkapigw.env" \
          "$BK_PKG_SRC_PATH"/"$MODULE_DIR_NAME"/support-files/templates/*
        ;;
    *)
        echo "Usage: $0 <模块>" >&2
