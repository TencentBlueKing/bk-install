...
GSE_AGENT_HOME=/usr/local/gse   # gse agent的默认安装家目录
NEED_RENDER=                    # 是否需要渲染agent配置，默认不渲染

# plugins pack with gse client/proxy 
BASE_PLUGINS=(
    basereport processbeat exceptionbeat
    gsecmdline bkmonitorbeat bkunifylogbeat
)
    ...
    for arch in x86_64 x86 aarch64; do
        ...
        if [[ -e $tmpdir/gse/agent_$m ]]; then
            ...
            if [[ -e "$tmpdir"/plugins_${m} ]]; then
                ...
                cp -a "$tmpdir"/plugins_${m}/bin/* plugins/bin/ # 拷贝脚本
                ...
                if [[ $NEED_RENDER -eq 1 ]]; then
                    (
                        ...
                        if [[ -n "$TAG" ]]; then
                            sed -i "s|/usr/local/gse|$GSE_AGENT_HOME|" plugins/etc/*
...
cp -a "$tmpdir"/plugins_linux_x86_64/bin/* plugins/bin/ # 拷贝脚本
