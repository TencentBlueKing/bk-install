...
CC_API_PARA="\"bk_biz_id\": $BLUEKING_BIZ_ID;\"page\": {\"start\": 0, \"limit\": 200, \"sort\": \"-name\"}"
JOSN_PAYLOAD=$(sed 's/;/,\n/g' <<<"$CC_API_PARA")

# 获取当前业务的服务模板列表
CUR_SVC_TPL=$($ESB_API_TOOL POST "${CC_API["LIST_SERVICE_TEMPLATE"]}" "$JOSN_PAYLOAD" | jq -r ' .data.info[] | [.id,.name] | @tsv')
# 获取当前业务的集群模板列表
