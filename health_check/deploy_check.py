...
class Storage_Check(object):
    ...
    def check_redis_by_get_mastername(self, module: str, 
        ...
        '''
        description:  根据master_name字段是否为None，判断redis集群部署种类
        '''                        
        ...
        if not IS_SENTINEL:
            try:
                logg.info("module -> [{}] check type is single !".format(module))
                pool = redis.Redis(host = single_host , port = single_port, password= single_password)
                version = pool.execute_command('INFO')['redis_version']
                pool.set('deploy_check1', 'Hollow, world')
                logg.info('module -> [{}] redis single slave node check success ! singel node is -> {}:{} pass: {}'.format(module, single_host, single_port, single_password))
                if re.search("5.0.\d", version):
                    logg.info("module -> [{}] redis single node version check success ! msg -> [{}]" .format(module, version))
                else:
                    logg.error("module -> [{}] redis single node version check failed! msg -> [{}]". format(module, version))
            except Exception as e:
                logg.error("module -> [{}] redis single check failed! error_msg -> [{}]".format(module, e))
        else:
            logg.info("module -> [{}] check type is sentinel !".format(module))
            try:
                try:
                    sentinel = Sentinel([(sentinel_host, sentinel_port)], sentinel_kwargs={'password': sentinel_password.replace('\'','')})
                except:
                    sentinel = Sentinel([(sentinel_host, sentinel_port)], sentinel_kwargs={'password': sentinel_password})
                singel_node = sentinel.discover_master(master_name)
                logg.info('module -> [{}] redis sentinel check success ! singel node is -> {}'.format(module, singel_node))
                pool = redis.Redis(host = singel_node[0], port = singel_node[1], password=single_password)
                version = pool.execute_command('INFO')['redis_version']
                pool.set('deploy_check1', 'Hollow, world')
                logg.info('module -> [{}] redis sentinel slave node check success ! singel node is -> {}'.format(module, singel_node))
                if re.search("5.0.\d", version):
                    logg.info("module -> [{}] redis sentinel  version check success ! msg -> [{}]" .format(module, version))
                else:
                    logg.error("module -> [{}] redis sentinel  version check failed! msg -> [{}]". format(module, version))
            except Exception as f:
                logg.error("module -> [{}] redis sentinel check failed! error_msg -> [{}]".format(module, f))
...
class Action(Storage_Check):
    ...
    def gse(self):
        ...
        self.zookeeper_check(
            ...
            module = 'gse',
            AUTH = gse_check_env_dict['BK_GSE_ZK_AUTH']
        )

        for ip in re.split(',', os.environ['BK_GSE_IP_COMMA'].replace('\'', "")):
            self.check_redis_by_get_mastername(
                module = "gse",
                single_host = ip,
                single_port = gse_check_env_dict['BK_GSE_REDIS_PORT'],
                single_password = gse_check_env_dict['BK_GSE_REDIS_PASSWORD'],
                IS_SENTINEL=False
            )
    ...
    def bkmonitorv3(self):
        ...
        self.zookeeper_check(
            module = 'monitorv3',
            host = monitorv3_check_env_dict['BK_GSE_ZK_HOST'] + ':' + monitorv3_check_env_dict['BK_GSE_ZK_PORT'],
        )
        self.elasticsearch7_check(
            module = 'monitorv3',
    ...
    def bknodeman(self):
        ...
        is_sentinel = False if nodeman_check_env_dict["BK_NODEMAN_REDIS_MASTER_NAME"] == None else True
        self.check_redis_by_get_mastername(
            module = 'bknodeman',
            ...
            master_name = nodeman_check_env_dict['BK_NODEMAN_REDIS_MASTER_NAME'],
            IS_SENTINEL = is_sentinel
        )
    ...
    def job(self):
        ...
        manage_sentinel_host, manage_sentinel_port= re.split(":", job_check_env_dict["BK_JOB_EXECUTE_REDIS_SENTINEL_NODES"])
        is_sentinel = True if job_check_env_dict['BK_JOB_MANAGE_REDIS_SENTINEL_MASTER'] != 'None' else False
        self.check_redis_by_get_mastername(
            sentinel_host = manage_sentinel_host, 
            sentinel_port = int(manage_sentinel_port),
        ...
        manage_sentinel_host, manage_sentinel_port= re.split(":", job_check_env_dict["BK_JOB_BACKUP_REDIS_SENTINEL_NODES"])
        is_sentinel = True if job_check_env_dict['BK_JOB_MANAGE_REDIS_SENTINEL_MASTER'] != 'None' else False
        self.check_redis_by_get_mastername(
            sentinel_host = manage_sentinel_host, 
            sentinel_port = int(manage_sentinel_port),
        ...
        manage_sentinel_host, manage_sentinel_port= re.split(":", job_check_env_dict["BK_JOB_CRONTAB_REDIS_SENTINEL_NODES"])
        is_sentinel = True if job_check_env_dict['BK_JOB_MANAGE_REDIS_SENTINEL_MASTER'] != 'None' else False
        self.check_redis_by_get_mastername(
            sentinel_host = manage_sentinel_host, 
            sentinel_port = int(manage_sentinel_port),
