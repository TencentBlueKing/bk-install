...
case $1 in 
    paas_plugins)
        ...
        printf "%s=%q\n" "BK_PAAS_PLUGINS_REDIS_PASSWORD" "$BK_PAAS_REDIS_PASSWORD"
    ...
    paas)
        ...
        gen_redis_password BK_PAAS "$(rndpw 12)"
        gen_mysql_password BK_PAAS "$(rndpw 12)"
        if [[ -r "$SELF_DIR/01-generate/dbadmin.env" ]];then
            source "$SELF_DIR/01-generate/dbadmin.env"
            source "$SELF_DIR/default/paas.env"
            printf "%s=%q\n" "BK_PAAS_ES7_ADDR" "${BK_PAAS_ES7_ADDR}"
        fi
        ;;
    cmdb) 
        set -a 
    ...
    job) 
        ...
        for m in BK_JOB_MANAGE BK_JOB_EXECUTE BK_JOB_CRONTAB BK_JOB_BACKUP BK_JOB_ANALYSIS; do
            ...
            gen_redis_password "$m" "$redis_password"
        done
    ...
    bkiam)
        ...
        mysql_pwd=$(rndpw 12)
        redis_pwd=$(rndpw 12)
        if [[ -z "$BK_SSM_MYSQL_PASSWORD" || "$BK_IAM_MYSQL_PASSWORD" ]]; then
            printf "%s=%q\n" "BK_SSM_MYSQL_PASSWORD" "$mysql_pwd"
            printf "%s=%q\n" "BK_IAM_MYSQL_PASSWORD" "$mysql_pwd"
        fi
        if [[ -z "$BK_SSM_REDIS_PASSWORD" || "$BK_IAM_MYSQL_PASSWORD" ]]; then
            gen_redis_password "BK_SSM" "${redis_pwd}"
    ...
    fta)
        if [[ -r "${SELF_DIR}"/04-final/paas.env ]];then
            source "${SELF_DIR}"/04-final/paas.env
            printf "%s=%q\n" BK_FTA_MYSQL_PASSWORD  $BK_PAAS_MYSQL_PASSWORD
        fi
        if [[ -z "$BK_NODEMAN_REDIS_PASSWORD" ]]; then
