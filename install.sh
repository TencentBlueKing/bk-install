...
install_bkenv () {
    ...
    for m in "${projects[@]}"; do
        ...
        if [[ ! -f ${HOME}/.tag/$m ]]; then
            case $module in
                global|license|bkssm|paasagent) : ;;
...
install_paas_plugins () {
    ...
    local python_path=/opt/py27/bin/python
    emphasize "install java8 on host: ${BK_PAAS_IP_COMMA}"
    "${SELF_DIR}"/pcmd.sh -m paas "${CTRL_DIR}/bin/install_java.sh -p ${INSTALL_PATH} -f ${BK_PKG_SRC_PATH}/java8.tgz"
...
case $module in
    paas|license|cmdb|job|gse|yum|consul|pypi|bkenv|rabbitmq|zk|mongodb|influxdb|license|cert|nginx|usermgr|appo|bklog|es7|python|appt|kafka|beanstalk|fta|nfs|dbcheck|controller|lesscode|node)
        install_"${module}" $@
        ;;
    paas_plugins)
        install_paas_plugins
